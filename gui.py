...
def filter_xlsx_files(xlsx_directory: Path, search_string: str, output_file: Path, callback=None) -> bool:
    """Filter rows containing search_string from all XLSX files in directory and consolidate"""
    try:
        if not search_string.strip():
            error_msg = "Search string cannot be empty"
            if callback:
                callback(error_msg)
            return False
        
        xlsx_files = list(Path(xlsx_directory).glob("*.xlsx"))
        if not xlsx_files:
            error_msg = f"No XLSX files found in {xlsx_directory}"
            if callback:
                callback(error_msg)
            return False
        ...
        for xlsx_file in xlsx_files:
            if callback:
                callback(f"Filtering: {xlsx_file.name}")
            ...
            try:
                ...
                if callback:
                    callback(f"⚠ Error filtering {xlsx_file.name}: {e}")
        
        new_wb.save(output_file)
        success_msg = f"✓ Filtered {len(xlsx_files)} files, found {total_rows_found} matching rows. Saved to {output_file}"
        if callback:
            callback(success_msg)
        return True
    
    except Exception as e:
        error_msg = f"Failed to filter XLSX files: {e}"
        if callback:
            callback(error_msg)
        return False
...
class PDFConverterGUI:
    ...
    def setup_ui(self):
        ...
        # Filter options
        filter_frame = ttk.LabelFrame(main_frame, text="Filter Options (Optional)", padding="10")
        filter_frame.grid(row=4, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(0, 10))
        filter_frame.columnconfigure(1, weight=1)
        
        ttk.Checkbutton(filter_frame, text="Enable Filtering after Conversion", 
                       variable=self.enable_filtering).grid(row=0, column=0, columnspan=3, pady=(0, 10))
        
        ttk.Label(filter_frame, text="Search String:").grid(row=1, column=0, sticky=tk.W, padx=(0, 10))
        ttk.Entry(filter_frame, textvariable=self.search_string).grid(row=1, column=1, columnspan=2, sticky=(tk.W, tk.E))
        ...
        try:
            ...
            if input_path.is_dir():
                ...
                if self.enable_filtering.get():
                    self.log(f"\n{'='*50}")
                    self.log("Starting filtering process...")
                    search_str = self.search_string.get().strip()
                    
                    if search_str:
                        filter_output = out_dir / "filtered_results.xlsx"
                        if filter_xlsx_files(out_dir, search_str, filter_output, self.log):
                            self.log(f"✅ Filtering complete")
                        else:
                            self.log(f"❌ Filtering failed")
                    else:
                        self.log("⚠ Filtering enabled but search string is empty")
